...
namespace Watchman.Engine.Tests.Sns
{
    ...
    public class SnsCreatorTests
    {
            ...
            string topic, string arn)
        {
            ...
            sns
                .Setup(x => x.CreateTopicAsync(topic, Arg.Any<CancellationToken>()))
                .Returns(response);
        }

        private void VerifyTopicCreated(IAmazonSimpleNotificationService sns)
        {
            sns.Verify(x => x.CreateTopicAsync(Arg.Any<string>(), Arg.Any<CancellationToken>()),
                Times.Once);
        }
        ...
        {
            sns.Verify(x => x.SubscribeAsync(Arg.Any<SubscribeRequest>(), Arg.Any<CancellationToken>()),
                Times.Never);
        }
    }
}
