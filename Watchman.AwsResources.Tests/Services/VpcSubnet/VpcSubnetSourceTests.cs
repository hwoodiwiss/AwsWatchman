...
namespace Watchman.AwsResources.Tests.Services.VpcSubnet
{
    ...
    public class VpcSubnetSourceTests
    {
        ...
        {
            _fakeEc2Client.Setup(x => x.DescribeSubnetsAsync(Arg.Any<CancellationToken>()))
        ...
        public async Task MultipleCalls_DataLoadedOnce()
        {
            ...
            // assert
            _fakeEc2Client.Verify(x => x.DescribeSubnetsAsync(Arg.Any<CancellationToken>()), Times.Once);
        }
    }
}
